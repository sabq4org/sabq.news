...
export default function EnglishCommentsPage() {
  ...
  const { data: allComments, isLoading } = useQuery<Comment[]>({
    queryKey: ["/api/en/dashboard/comments"],
  });
  ...
  const approveMutation = useMutation({
    mutationFn: async (commentId: string) => {
      return await apiRequest(`/api/en/dashboard/comments/${commentId}/approve`, {
        method: "POST",
      });
    },
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/en/dashboard/comments"] });
    },
    onError: () => {
      toast({
        title: "Error",
  ...
  const rejectMutation = useMutation({
    mutationFn: async ({ commentId, reason }: { commentId: string; reason?: string }) => {
      return await apiRequest(`/api/en/dashboard/comments/${commentId}/reject`, {
        method: "POST",
  ...
  const restoreMutation = useMutation({
    mutationFn: async (commentId: string) => {
      return await apiRequest(`/api/en/dashboard/comments/${commentId}/restore`, {
        method: "POST",
      });
    },
    onSuccess: () => {

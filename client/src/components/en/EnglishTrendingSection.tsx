...
const modes = [
  { value: 'daily' as TrendMode, label: 'Daily Pulse', icon: TrendingUp },
  { value: 'personalized' as TrendMode, label: 'For You', icon: Target },
  { value: 'predictive' as TrendMode, label: 'What\'s Next', icon: Sparkles },
];
...
function TrendingArticleCard({ article, index }: { article: TrendingArticle; index: number }) {
  ...
  return (
    <Link href={`/en/article/${article.slug}`} data-testid={`link-trending-${article.id}`}>
      <Card 
      ...
      >
        {article.imageUrl && (
          <div className="relative h-48 overflow-hidden">
            ...
            {/* Rank Badge */}
            {article.rank && (
              <div className="absolute top-3 left-3 bg-primary text-primary-foreground w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm shadow-lg">
                #{article.rank}
              </div>
            )}
...
function EmptyState({ mode }: { mode: TrendMode }) {
  if (mode === 'personalized') {
    return (
      ...
      >
        ...
        <div>
          <h3 className="text-lg font-semibold mb-2">Sign in to see personalized recommendations</h3>
          <p className="text-sm text-muted-foreground">
            Get articles tailored to your interests and reading history
          </p>
        </div>
        <Link href="/en/login" data-testid="link-trending-login">
  ...
  return (
    ...
    >
      ...
      <div>
        <h3 className="text-lg font-semibold mb-2">No trending articles right now</h3>
        <p className="text-sm text-muted-foreground">
          Check back later for the latest trending stories
        </p>
      </div>
    </div>
  );
...
export function EnglishTrendingSection() {
  const [mode, setMode] = useState<TrendMode>('daily');

  const { data, isLoading } = useQuery<TrendingData>({
    queryKey: ['/api/en/trends', mode],
    queryFn: async () => {
      const res = await fetch(`/api/en/trends?mode=${mode}&limit=10`, {
        credentials: 'include',
      });
  ...
  return (
    <section className="space-y-4" data-testid="section-trending">

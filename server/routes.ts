...
`;
  ...
  app.get("/api/ur/smart-blocks/query/articles", async (req: any, res) => {
    try {
      ...
      if (!keyword) {
        return res.status(400).json({ message: "مطلوبہ لفظ ضروری ہے" });
      }

      console.log(`🔍 [UR Smart Block] Searching for keyword: "${keyword}", limit: ${limit}`);
      ...
      if (categories) {
        const categoryList = Array.isArray(categories) ? categories : [categories];
        conditions.push(inArray(urArticles.categoryId, categoryList));
      }

      if (dateFrom) {
        conditions.push(gte(urArticles.publishedAt, new Date(dateFrom as string)));
      }

      if (dateTo) {
        conditions.push(gte(urArticles.publishedAt, new Date(dateTo as string)));
      }
      ...
      console.log(`✅ [UR Smart Block] Found ${articles.length} articles for "${keyword}"`);
      articles.forEach((article, index) => {
        console.log(`   ${index + 1}. "${article.title.substring(0, 60)}..."`);
      });
      ...
      console.error("❌ [UR Smart Block] Error querying articles:", error);
      res.status(500).json({ message: "مضامین تلاش کرنے میں ناکامی" });
    }
  });

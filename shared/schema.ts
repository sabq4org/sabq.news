...
// Chat Channels - القنوات/المحادثات
export const chatChannels = pgTable("chat_channels", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  type: text("type").notNull(), // 'direct', 'group', 'channel'
  name: text("name"), // nullable for direct messages
  description: text("description"),
  category: text("category"), // 'desk', 'department', 'team', 'custom'
  departmentType: text("department_type"), // 'local', 'world', 'sports', etc.
  ...
  index("chat_channels_status_idx").on(table.status),
  index("chat_channels_created_by_idx").on(table.createdBy),
  index("chat_channels_category_idx").on(table.category),
]);

// Chat Members - أعضاء القنوات
export const chatMembers = pgTable("chat_members", {
  id: varchar("id").default(sql`gen_random_uuid()`).notNull(),
  channelId: varchar("channel_id").references(() => chatChannels.id, { onDelete: "cascade" }).notNull(),
  userId: varchar("user_id").references(() => users.id).notNull(),
  ...
  notificationPreference: text("notification_preference").default("all").notNull(), // 'all', 'mentions', 'none'
}, (table) => ({
  pk: primaryKey({ columns: [table.channelId, table.userId] }),
  channelIdx: index("chat_members_channel_idx").on(table.channelId),
...
export const chatMessages = pgTable("chat_messages", {
  ...
  userId: varchar("user_id").references(() => users.id).notNull(),
  content: text("content").notNull(),
  contentType: text("content_type").notNull().default("text"), // 'text', 'rich_text', 'system'
  parentMessageId: varchar("parent_message_id"), // self-reference for threads
  ...
  index("chat_messages_pinned_idx").on(table.isPinned),
  index("chat_messages_scheduled_idx").on(table.scheduledFor),
]);
...
export const chatAttachments = pgTable("chat_attachments", {
  ...
  type: text("type").notNull(), // 'image', 'video', 'audio', 'voice_note', 'document', 'file'
  fileName: text("file_name").notNull(),
  ...
  index("chat_attachments_type_idx").on(table.type),
]);
...
export const chatReactions = pgTable("chat_reactions", {
  ...
  userIdx: index("chat_reactions_user_idx").on(table.userId),
}));

// Chat Mentions - الإشارات
export const chatMentions = pgTable("chat_mentions", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  messageId: varchar("message_id").references(() => chatMessages.id, { onDelete: "cascade" }).notNull(),
  mentionedUserId: varchar("mentioned_user_id").references(() => users.id).notNull(),
  mentionType: text("mention_type").notNull().default("user"), // 'user', 'channel', 'all'
  createdAt: timestamp("created_at").defaultNow().notNull(),
}, (table) => [
  index("chat_mentions_mentioned_user_created_idx").on(table.mentionedUserId, table.createdAt),
  index("chat_mentions_message_idx").on(table.messageId),
]);

// Chat Read Receipts - إيصالات القراءة
export const chatReadReceipts = pgTable("chat_read_receipts", {
  ...
  messageIdx: index("chat_read_receipts_message_idx").on(table.messageId),
  userIdx: index("chat_read_receipts_user_idx").on(table.userId),
}));

// Chat Pinned Messages - الرسائل المثبتة
export const chatPinnedMessages = pgTable("chat_pinned_messages", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  channelId: varchar("channel_id").references(() => chatChannels.id, { onDelete: "cascade" }).notNull(),
  messageId: varchar("message_id").references(() => chatMessages.id, { onDelete: "cascade" }).notNull(),
  pinnedBy: varchar("pinned_by").references(() => users.id).notNull(),
  pinnedAt: timestamp("pinned_at").defaultNow().notNull(),
  displayOrder: integer("display_order").default(0).notNull(),
}, (table) => [
  index("chat_pinned_messages_channel_idx").on(table.channelId),
  index("chat_pinned_messages_message_idx").on(table.messageId),
  uniqueIndex("chat_pinned_messages_unique_idx").on(table.channelId, table.messageId),
]);

// Chat Moderation Logs - سجلات الإشراف
export const chatModerationLogs = pgTable("chat_moderation_logs", {
  ...
  userId: varchar("user_id").references(() => users.id).notNull(), // المشرف
  targetUserId: varchar("target_user_id").references(() => users.id), // المستهدف
  action: text("action").notNull(), // 'delete_message', 'pin', 'unpin', 'mute_user', 'remove_member', 'slow_mode', etc.
  reason: text("reason"),
  metadata: jsonb("metadata").$type<{
    previousValue?: any;
    newValue?: any;
  ...
  index("chat_moderation_logs_channel_idx").on(table.channelId),
  index("chat_moderation_logs_user_idx").on(table.userId),
  index("chat_moderation_logs_target_user_idx").on(table.targetUserId),
  index("chat_moderation_logs_action_idx").on(table.action),
  index("chat_moderation_logs_created_idx").on(table.createdAt),
]);

// Chat Retention Policies - سياسات الاحتفاظ
export const chatRetentionPolicies = pgTable("chat_retention_policies", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  channelId: varchar("channel_id").references(() => chatChannels.id, { onDelete: "cascade" }), // null for global policy
  ...
  index("chat_retention_policies_channel_idx").on(table.channelId),
...
export const chatChannelsRelations = relations(chatChannels, ({ one, many }) => ({
  ...
  pinnedMessages: many(chatPinnedMessages),
  retentionPolicy: one(chatRetentionPolicies, {
    fields: [chatChannels.id],
    references: [chatRetentionPolicies.channelId],
  }),
}));
...
export const chatMembersRelations = relations(chatMembers, ({ one }) => ({
  ...
  lastReadMessage: one(chatMessages, {
    fields: [chatMembers.lastReadMessageId],
...
export const chatMessagesRelations = relations(chatMessages, ({ one, many }) => ({
  ...
  readReceipts: many(chatReadReceipts),
}));
...
export const chatModerationLogsRelations = relations(chatModerationLogs, ({ one }) => ({
  channel: one(chatChannels, {
    fields: [chatModerationLogs.channelId],
    references: [chatChannels.id],
  }),
  message: one(chatMessages, {
    fields: [chatModerationLogs.messageId],
    references: [chatMessages.id],
  }),
  moderator: one(users, {
    fields: [chatModerationLogs.userId],
    references: [users.id],
  }),
  targetUser: one(users, {
    fields: [chatModerationLogs.targetUserId],
    references: [users.id],
  }),
}));

export const chatRetentionPoliciesRelations = relations(chatRetentionPolicies, ({ one }) => ({
  channel: one(chatChannels, {
    fields: [chatRetentionPolicies.channelId],
    references: [chatChannels.id],
  }),
  createdByUser: one(users, {
    fields: [chatRetentionPolicies.createdBy],
    references: [users.id],
  }),
}));
...
export type ChatMemberWithDetails = ChatMember & {
  channel?: ChatChannel;
  user?: User;
};

// ============================================
// Chat System Zod Schemas
// ============================================
...
export const insertChatChannelSchema = createInsertSchema(chatChannels).omit({
  ...
  name: z.string().min(1, "اسم القناة مطلوب").max(100, "اسم القناة يجب ألا يتجاوز 100 حرف").optional(),
  category: z.enum(["desk", "department", "team", "custom"]).optional(),
  departmentType: z.enum(["local", "world", "sports", "economy", "technology", "health", "culture", "entertainment"]).optional(),
  status: z.enum(["active", "archived", "deleted"]).default("active"),
});

export const updateChatChannelSchema = insertChatChannelSchema.partial();
...
export const updateChatMemberSchema = insertChatMemberSchema.partial().omit({
  channelId: true,
  userId: true,
});
...
export const insertChatMessageSchema = createInsertSchema(chatMessages).omit({
  ...
  contentType: z.enum(["text", "rich_text", "system"]).default("text"),
});

export const updateChatMessageSchema = createInsertSchema(chatMessages).pick({
  content: true,
  metadata: true,
}).extend({
  content: z.string().min(1, "محتوى الرسالة مطلوب").max(10000, "الرسالة طويلة جداً"),
...
export const insertChatAttachmentSchema = createInsertSchema(chatAttachments).omit({
  ...
  type: z.enum(["image", "video", "audio", "voice_note", "document", "file"]),
...
export const insertChatReactionSchema = createInsertSchema(chatReactions).omit({
  ...
  emoji: z.string().min(1, "الرمز التعبيري مطلوب").max(10, "الرمز التعبيري طويل جداً"),
});
...
export const insertChatMentionSchema = createInsertSchema(chatMentions).omit({
  ...
  mentionType: z.enum(["user", "channel", "all"]).default("user"),
});
...
export const updateChatPinnedMessageSchema = insertChatPinnedMessageSchema.pick({
  displayOrder: true,
});
...
export const insertChatRetentionPolicySchema = createInsertSchema(chatRetentionPolicies).omit({
  ...
  retentionDays: z.number().min(1, "يجب أن تكون فترة الاحتفاظ يوم واحد على الأقل").max(365, "فترة الاحتفاظ يجب ألا تتجاوز 365 يوم"),
});

export const updateChatRetentionPolicySchema = insertChatRetentionPolicySchema.partial().omit({
  channelId: true,
  createdBy: true,
});
...
export type UpdateChatChannel = z.infer<typeof updateChatChannelSchema>;

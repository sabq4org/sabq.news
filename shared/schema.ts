...
// ============================================
// DECISION DASHBOARD DTO - لوحة القرار الذكية
// ============================================

// حالة المؤشر (أخضر/أصفر/أحمر)
export type MetricStatus = 'positive' | 'neutral' | 'negative';

// مؤشر مع مقارنة
export const metricWithComparisonSchema = z.object({
  value: z.number(),
  previousValue: z.number().optional(),
  percentChange: z.number().optional(), // نسبة التغير
  status: z.enum(['positive', 'neutral', 'negative']),
  label: z.string(),
  comparisonLabel: z.string().optional(), // "عن أمس" أو "عن متوسط الأسبوع"
});
export type MetricWithComparison = z.infer<typeof metricWithComparisonSchema>;

// الملخص التنفيذي العلوي
export const executiveSummarySchema = z.object({
  engagement: metricWithComparisonSchema, // التفاعل مقارنة بأمس
  reads: metricWithComparisonSchema, // القراءات مقارنة بمتوسط الأسبوع
  underperformingCount: z.number(), // عدد المقالات ضعيفة الأداء
  trendingArticle: z.object({
    id: z.string(),
    title: z.string(),
    views: z.number(),
  }).nullable(), // مقال ترند الآن
});
...
export const actionRecommendationSchema = z.object({
  id: z.string(),
  type: z.enum(['review_title', 'republish', 'content_gap', 'boost', 'moderate', 'urgent']),
  icon: z.string(), // اسم أيقونة lucide
  title: z.string(),
  ...
  metric: z.string().optional(), // مثال: "CTR منخفض 2.3%"
});
export type ActionRecommendation = z.infer<typeof actionRecommendationSchema>;

// مقال مع سبب النجاح/الفشل
export const articlePerformanceSchema = z.object({
  id: z.string(),
  title: z.string(),
  views: z.number(),
  engagement: z.number(), // عدد التفاعلات
  ctr: z.number().optional(), // Click-Through Rate
  publishedAt: z.string(),
  category: z.string().optional(),
  author: z.string().optional(),
  successReason: z.string().optional(), // سبب النجاح
  status: z.enum(['positive', 'neutral', 'negative']),
});
export type ArticlePerformance = z.infer<typeof articlePerformanceSchema>;

// نشاط أحد أعضاء الفريق
export const teamMemberActivitySchema = z.object({
  userId: z.string(),
  name: z.string(),
  avatar: z.string().optional(),
  publishedToday: z.number(), // عدد المقالات المنشورة اليوم
  pendingArticles: z.number(), // مقالات معلقة
  avgPublishTime: z.number().optional(), // متوسط وقت النشر بالدقائق
  lastActive: z.string().optional(),
});
export type TeamMemberActivity = z.infer<typeof teamMemberActivitySchema>;

// نشاط الفريق الإجمالي
export const teamActivitySchema = z.object({
  totalPublishedToday: z.number(),
  totalPendingArticles: z.number(),
  avgTeamPublishTime: z.number().optional(),
  topPublisher: teamMemberActivitySchema.optional(), // أكثر من نشر
  members: z.array(teamMemberActivitySchema),
});
...
// البريف اليومي (صباح سبق)
export const dailyBriefSchema = z.object({
  greeting: z.string(),
  date: z.string(),
  opportunities: z.array(insightSchema), // فرص المحتوى
  risks: z.array(insightSchema), // المخاطر
  boostCandidate: z.object({
    articleId: z.string(),
    title: z.string(),
    reason: z.string(),
  }).nullable(), // مقال يستحق الدفع
  summary: z.string(), // ملخص نصي قصير
});
export type DailyBrief = z.infer<typeof dailyBriefSchema>;

// الاستجابة الكاملة للـ Dashboard الذكي
export const decisionDashboardResponseSchema = z.object({
  executiveSummary: executiveSummarySchema,
  actionRecommendations: z.array(actionRecommendationSchema),
  topArticles: z.array(articlePerformanceSchema),
  underperformingArticles: z.array(articlePerformanceSchema),
  teamActivity: teamActivitySchema,
  insights: z.array(insightSchema),
  dailyBrief: dailyBriefSchema,
  lastUpdated: z.string(),
});
export type DecisionDashboardResponse = z.infer<typeof decisionDashboardResponseSchema>;

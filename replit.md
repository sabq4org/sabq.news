...
- Enhanced SEO and Social Sharing: Added comprehensive Open Graph and Twitter Card meta tags to client/index.html for proper homepage sharing on WhatsApp/Facebook/Twitter with logo, title, description, and image dimensions. Implemented server-side rendering solution via socialCrawler.ts middleware that detects social media crawlers (WhatsApp, Facebook, Twitter, Telegram, etc.) and serves static HTML with proper meta tags for article pages, solving the SPA meta tag problem. Middleware intercepts crawler requests before Vite/static setup and generates optimized HTML with article-specific og:image, og:title, and og:description.

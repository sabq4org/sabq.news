...
-   **iFox Public Articles Page:** Public-facing page at `/ifox` displaying all AI-generated articles. Uses `GET /api/ifox/articles` endpoint that filters published articles by `aiGenerated === true` flag (regardless of category). Features responsive grid layout (3/2/1 columns), RTL-friendly design, article cards with featured images, category badges, AI generation indicators, excerpts, Arabic date formatting, and view counts. Includes loading/error/empty states with proper data-testid attributes for testing.

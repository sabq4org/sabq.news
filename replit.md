...
-   **Audio Newsletter System:** Comprehensive audio broadcasting platform with ElevenLabs Flash v2.5 integration for Arabic TTS, automated daily briefings (6 AM morning, 8 PM evening), 8 newsletter templates, advanced editor with drag-drop article ordering, homepage floating audio widget, public newsletter page (`/audio-newsletters`), RSS podcast feed, comprehensive analytics dashboard with device tracking and CSV export, and automated scheduling system with email notifications. **Recent improvements (Nov 23, 2025):** Fixed critical audio generation errors by correcting timestamp field handling - changed all `toISOString()` calls to direct Date objects for timestamp columns (`publishedAt`, `startedAt`). Enhanced scheduling reliability with `scheduledFor` timestamp column and `metadata` JSONB column for retry tracking and recurrence settings. Added robust date validation (`parseValidDate()` helper) to prevent Invalid Date crashes in all date-range API queries. Restored full retry mechanism (3 attempts max) and recurring newsletter logic using metadata fields. Fixed date filtering in analytics queries by passing Date objects directly instead of ISO strings.

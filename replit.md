...
-   **AI-Powered Comment Sentiment Analysis (November 2025):** Enterprise-grade sentiment analysis system for comment moderation with Claude Sonnet 4-5 (primary) and Gemini Flash (fallback). Features include:
    - **Real-time Analysis:** Single comment and batch processing (up to 100 comments, concurrency 3) with automatic language detection (Arabic, English, Urdu)
    - **Sentiment Classification:** Positive, neutral, or negative with confidence scores (0-1 scale)
    - **Historical Tracking:** Complete audit trail stored in `comment_sentiments` table with provider, model, and metadata
    - **Dashboard Integration:** Sentiment filter in DashboardComments, one-click analysis buttons, and visual SentimentIndicator component
    - **Analytics Dashboard:** Comprehensive visualization with pie charts (distribution), line charts (30-day trends), and top articles by sentiment metrics

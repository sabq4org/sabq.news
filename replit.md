...
-   **Native Ads Daily Budget System:** Self-service advertising platform with CPC (cost-per-click) billing, daily budget limits in halalas (Saudi cents), automatic budget enforcement stopping ad serving when limit reached, and scheduled midnight reset job (21:00 UTC = 00:00 Saudi time). Includes advertiser dashboard alerts for exhausted budgets, admin indicators, and daily spend tracking via `nativeAdDailySpend` table.

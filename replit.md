...
-   **iFox Public Articles Page:** Public-facing page at `/ifox` displaying all AI-generated articles with URL-based category filtering. Uses `GET /api/ifox/articles?categorySlug={slug}` endpoint that filters published articles by specific iFox category (ai-news, ai-insights, ai-opinions, ai-tools, ai-voice) and `aiGenerated === true` flag. Features sticky category tabs, responsive grid layout (3/2/1 columns), RTL-friendly design, article cards with featured images, category badges, AI generation indicators, excerpts, Arabic date formatting, and view counts. Includes loading/error/empty states with proper data-testid attributes for testing. November 2024: Fixed API response parsing to extract articles from `{ articles: [...], total: number }` response structure.

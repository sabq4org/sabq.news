...
## Recent Changes
**November 8, 2025:**
- Fixed Email Verification Bug: Corrected VerifyEmail.tsx component to properly handle apiRequest responses. The bug was caused by checking `response.ok` on a direct data object instead of a Response object, which always failed and showed error message. Now correctly uses try/catch pattern where successful apiRequest returns data directly. Also fixed query key from `/api/user` to `/api/auth/user`. Email verification now works correctly - users can click verification link and see success message.
- Changed Email Template Icon: Updated verification email header icon from newspaper 📰 to brain 🧠 to better represent the AI-powered nature of Sabq Smart platform.
- Added Email Verification Alert to Profile Page: Unverified users now see a prominent yellow warning banner in their profile page with clear messaging "يرجى تفعيل حسابك عبر البريد الإلكتروني" and a button to resend verification email. Alert uses AlertCircle icon (no emoji per project rules), displays only when user.emailVerified === false, and includes loading state during resend operation. Banner positioned between hero section and main content for maximum visibility.
- Fixed Email Verification Links: Configured FRONTEND_URL environment variable to use production domain (https://sabq.life) instead of localhost. Email service now automatically detects correct domain from FRONTEND_URL → REPLIT_DOMAINS → localhost fallback, ensuring verification links work in production. Added logging to show which URL is being used for email links.
- Improved Email Verification UX: Enhanced 403 error message from "يرجى تفعيل حسابك عبر البريد الإلكتروني للوصول الكامل للميزات." to cleaner "يرجى تفعيل حسابك عبر البريد الإلكتروني أولاً". Added special 403 error handling in queryClient.ts to display user-friendly toast notifications instead of raw JSON responses, improving user experience during unverified account interactions.
- Implemented Email Verification System with SendGrid: Created complete email verification flow with server/services/email.ts handling SendGrid integration, database schema for email_verification_tokens table, three API endpoints (/api/auth/verify-email, /api/auth/resend-verification, auto-send on registration), and VerifyEmail page with loading/success/error states. System sends bilingual (Arabic/English) HTML emails with verification links valid for 24 hours, automatically updates emailVerified status, and integrates seamlessly with user registration flow. 

...
-   **WhatsApp Auto-Publish System:** Enterprise-grade WhatsApp-to-article publishing with Twilio API integration, AI content processing, multi-image management (Google Cloud Storage), and intelligent content cleaning. Includes **Message Aggregation System** that handles Twilio's 4,096 character limit via atomic SQL upsert operations (`INSERT ... ON CONFLICT ... DO UPDATE SET message_parts = array_cat(...)`), with a 30-second merge window for multi-part messages. Background aggregator job runs every 5 seconds, processes expired pending messages, and supports the "إرسال" command for immediate processing. Uses `pending_whatsapp_messages` table with unique constraint on (phone_number, token) for race condition prevention.
